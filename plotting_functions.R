...
plot_traitlag <- function(dat, level="resource") {
  ...
  p <- dat %>%
    ## filter for times between start and end of climate change (1000<=t<=1300):
    filter(time>=1000, time<=1300) %>%
    ...
    ## calculate local temperatures (see the Temp() function in ecoevo_main.R):
    mutate(temp=(25+10)*seq(from=0, to=1, l=50)[patch]-
             10+((1.26-9.66)*seq(from=0, to=1, l=50)[patch]+9.66)*
             smoothstep((time-1000)/300)) %>%
