...
plot_traitlag <- function(dat) {
  p <- dat %>%
    filter(time>=1000, time<=1300, tl=="R") %>%
    mutate(n=ifelse(n<=0, 0, n)) %>%
    mutate(parameterization=paste0("vbar=", vbar, ", dbar=", dbar)) %>%
    mutate(model=factor(model, ordered=TRUE,
                        levels=c("baseline", "trophic", "T-dep.",
                                 "T-dep. + trophic"))) %>%
    mutate(temp=Temp(seq(from=0, to=1, l=50)[patch], time, t0, tE, Cmax, Cmin,
                     Tmax, Tmin)) %>%
    group_by(time, replicate, parameterization, model) %>%
    mutate(reldens=n/sum(n)) %>%
    ungroup %>%
    group_by(time, patch, replicate, parameterization, model) %>%
    summarise(A=mean(reldens*(m-temp)^2), mbar=mean(reldens*m),
              sig=mean(reldens*(m-mbar)^2)) %>%
    ungroup %>%
    group_by(replicate, parameterization, model) %>%
    summarise(mA=mean(A), msig=mean(sig)) %>%
    ungroup %>%
    ggplot +
    aes(x=msig, y=mA, colour=model, shape=parameterization) +
