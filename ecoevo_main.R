...
## $ Rscript ecoevo.R [vbar] [dbar] [structure] [replicate] [outfile]
...
if (length(clargs)>0) { ## command-line arguments
    ...
    structure <- clargs[4] ## "baseline", "trophic", "Tdep", or "Tdeptrophic"
...
eqs <- function(time, state, pars) {
    ...
    if (structure %in% c("Tdep", "Tdeptrophic")) {
...
if (structure %in% c("trophic", "Tdeptrophic")) SC <- S ## ...consumer species
...
if (structure %in% c("trophic", "Tdeptrophic")) {
...
eta <- 1 ## competition width (centigrade; only for Tdep and Tdeptrophic)
...
## coerce model parameters into list
params <- list(SR=SR, SC=SC, S=S, L=L, rho=rho, kappa=kappa, a=a, eta=eta,
               ...
               tE=tE, t0=t0, mig=mig, structure=structure)
...
dat <- ode(func=eqs, y=c(ninit,muinit), times=time, parms=params, method="rk4") %>%
    ...
    organize_data(times=seq(from=0, to=tmax, by=100), pars=params) %>%
    ## add replicate, genetic var., dispersal rate, and structure as new columns
    mutate(replicate=replicate, vbar=vbar, dbar=dbar, structure=structure)


## ---------------------------- generate output -----------------------------------


if (outfile!="") write_csv(dat, path=outfile) ## save data if file was specified
