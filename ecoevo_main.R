...
eqs <- function(time, state, pars) {
  ...
  for (i in 1:pars$S) {
    mign[i,] <- pars$d[i]*as.numeric(pars$mig%*%n[i,]) # sum_l mig_ikl*n_il
    summig[i,] <- pars$d[i]*as.numeric(rep(1, pars$L)%*%pars$mig) # sum_l mig_ilk
    mnmu[i,] <- pars$d[i]*as.numeric(pars$mig%*%
  ...
  dndt[,1] <- dndt[,1]+pars$d*(pars$mig[1,2]*n[,2] - pars$mig[2,1]*n[,1])
  dndt[,pars$L] <- dndt[,pars$L]+pars$d*
    (pars$mig[pars$L,pars$L-1]*n[,pars$L-1]-
       pars$mig[pars$L-1,pars$L]*n[,pars$L])
  dmdt[,1] <- dmdt[,1]+h2[,1]*pars$d*pars$mig[1,2]*n[,2]*ninv[,1]*(m[,2]-m[,1])
  dmdt[,pars$L] <- dmdt[,pars$L]+h2[,pars$L]*pars$d*pars$mig[pars$L,pars$L-1]*
...
pars <- list(SR=SR, SC=SC, S=S, L=L, rho=rho, kappa=kappa, a=a, eta=eta,
             ...
             tE=tE, t0=t0, d=d, mig=mig, model=model)
